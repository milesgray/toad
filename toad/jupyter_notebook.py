...
def _get_file_name_from_path(input_path):
    """ A simple utility to extract the name of a file without the file extension from a given path.
    ...
    Args:
        input_path (string):
            path with a file name in it

    Returns:
        file name only from the path
    """
...
def _flatten(obj):
    ...
    Args:
        obj (None, dict, luigi.six.string_types, iter):
            object instance to be flattened, should be a nested list-style object
...
def _get_path(obj):
    """Extracts the `path` attribute from `obj` if available; otherwise, simply returns None.
...
def _get_path_from_collection(coll):
    """Extracts the `path` attribute from atomic objects or objects organized in a collection (dict or iterable) wherever the `path` attribute is available (otherwise extracts `None`).
