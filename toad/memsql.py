...
#
"""
Support for SQLAlchmey. Provides MemSQLAlchemyTarget for storing in databases
...
class MemSQLAlchemyTarget(toad.targets.DataTarget):
    """
    MemSQL Database target using SQLAlchemy.

    This will rarely have to be directly instantiated by the user.

    Typical usage would be to override `toad.memsql.CopyToTable` class
    ...
    def __init__(self, connection_string, target_table, update_id, echo=False, connect_args=None):
        """
        Constructor for the MemSQLAlchemyTarget.
    ...
    def engine(self):
        ...
        """
        pid = os.getpid()
        conn = MemSQLAlchemyTarget._engine_dict.get(self.connection_string)
        if not conn or conn.pid != pid:
            ...
            MemSQLAlchemyTarget._engine_dict[self.connection_string] = self.Connection(
                engine, pid)
        return MemSQLAlchemyTarget._engine_dict[self.connection_string].engine
    ...
    def create_marker_table(self):
        ...
        if self.marker_table is None:
            self.marker_table = luigi.configuration.get_config().get(
                'sqlalchemy', 'marker-table', 'table_updates')
        ...
        with engine.begin() as con:
            ...
            if not con.dialect.has_table(con, self.marker_table):
                self.marker_table_bound = sqlalchemy.Table(
                    ...
                    sqlalchemy.Column(
                        "update_id", sqlalchemy.String(128), primary_key=True),
    ...
    def open(self, mode):
        raise NotImplementedError("Cannot open() MemSQLAlchemyTarget")


class MemSQLNativeTarget(MemSQLAlchemyTarget):
    """
    MemSQL Database target using the memsql-python repo released by the authors.

    This takes advantage of muliprocessing, so make sure your records can be inserted in random order.
...
class CopyToTable(toad.tasks.EventTask):
    """
    An abstract task for inserting a data set into MemSQL with SQLAlchemy
    ...
    * optionally override the `schema` attribute to use a different schema for
    ...
    cfg = toad.config.memsql()

    @abc.abstractproperty
    def connection_string(self):
        return self.cfg.faces_connection_string
    ...
    column_separator = luigi.Parameter(default="\t", description="How columns are separated in file")
    chunk_size = luigi.IntParameter(default=5000, description="Number of records to insert with each statement")
    start_index = luigi.IntParameter(default=0, description="Line to start at, skip all until this. 0 default effectively disables")
    end_index = luigi.IntParameter(default=-1, description="Line to end at, end at this line. -1 default disables limit")
    batch_size = luigi.IntParameter(default=-1, description="Number of records to insert during each batch (transaction). -1 default disables")
    batch_amount = luigi.IntParameter(default=1, description="Number of batches to insert")
    # Set this to true only if the table has already been created by alternate means
    reflect = False
    ...
    def create_table(self, engine):
        ...
        needs_setup = (len(self.columns) == 0) or (
            False in [len(c) == 2 for c in self.columns]) if not self.reflect else False
        if needs_setup:
            ...
            with engine.begin() as con:
                ...
                try:
                    if not con.dialect.has_table(con, self.table, self.schema or None):
                        ...
                        self.table_bound = sqlalchemy.Table(
                            self.table, metadata, *sqla_columns)
                        metadata.create_all(engine)
                    else:
                        full_table = '.'.join(
    ...
    def rows(self):
        ...
        with self.input().open('r') as f:
            for line_index, line in enumerate(f):
                if line_index < self.start_index:
                    continue
                if self.end_index > 0 and line_index >= self.end_index:
                    break
                yield line.strip("\n").split(self.column_separator)

    def run(self):
        self.start("Running task copy to table for update id %s for table %s" % (self.update_id(), self.table))
        output = self.output()
        ...
        for i in range(self.batch_amount):
            ...
            with engine.begin() as conn:                
                ...
                while ins_rows:
                    ...
                    ins_rows = [dict(zip(("_" + c.key for c in self.table_bound.c), row))
                                for row in itertools.islice(rows, self.chunk_size)]
                    self.debug("Finished inserting %d rows into SQLAlchemy target" % len(ins_rows))                    
                    batch_count += len(ins_rows)

                    if self.batch_size > 0 and batch_count > self.batch_size:
                        break

            self.debug("Done with batch {} of {}".format(i, self.batch_amount))

        output.touch()
        self.final("Finished inserting rows into SQLAlchemy target")
